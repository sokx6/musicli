...
实测会出现 base64 作为普通文本泄漏到屏幕的情况。分块传输可以把
单个 APC 的 payload 控制在较小范围内，规避这类长 payload 与后续
renderer 输出交错时的解析风险。
...
`s.buf` 后。实测中，如果 APC payload 过大，紧随其后的 SGR diff
更容易触发 base64 明文泄漏。
...
分块前：一个 200KB 的 APC 作为单条消息写入 TTY。实测中，当这类
长 payload 与 renderer diff 输出处在同一次刷新附近时，容易出现
payload 截断或 base64 泄漏为明文。

分块后：每个 APC chunk 最多 4096 字节，kitty 按规范通过 `m=1/m=0`
重组 payload。较小的 chunk 减少了单次 APC payload 的长度，也让
payload 边界更明确，从而规避实测到的 base64 泄漏问题。
...
分块传输降低了 `kittyCoverCmd()` 产生的 APC 与 diff 输出在同一次
刷新附近写入时出现 base64 泄漏的风险。
