...
- `[lyrics] highlight_mode = "played" | "current"` 控制逐字歌词高亮范围，默认 `played`；`h` 在两种模式间切换，当前字开始即视为已播放
...
- 默认不可删除的 `Favorites` 歌单；`f` 快速收藏/取消收藏当前列表中选中的歌曲
- 收藏歌曲在曲目标题前显示固定宽度的 `*`，避免 CJK 终端中的模糊字符宽度
